...
const kmpcLinLongGurobi = GPSKinMPCPathFollowerFrenetLinLongGurobi  # remaining
const kmpcLinLatGurobi = GPSKinMPCPathFollowerFrenetLinLatGurobi 	# experimental
...
# 	x_mpc = zeros(kmpcLinLongGurobi.N+1)
# 	y_mpc = zeros(kmpcLinLongGurobi.N+1)
# 	psi_mpc = zeros(kmpcLinLongGurobi.N+1)
# 	v_mpc = zeros(kmpcLinLongGurobi.N+1)
# 	bta = zeros(kmpcLinLongGurobi.N)
# 	# init initial state
# 	x_mpc[1] = x_curr
# 	y_mpc[1] = y_curr
# 	psi_mpc[1] = psi_curr
...
# 			solv_time_long_gurobi1_all[it_num+1] = solv_time_long_gurobi1
			ey_curr = 0.017048203834592116
			epsi_curr = -0.0560848810781025
			df_opt = -0.4591839740073548
			s_pred_gurobi = [564.14, 564.53, 564.91, 565.282, 565.645, 566.001, 566.351, 566.696, 567.037, 567.374, 567.708, 568.039, 568.369, 568.697, 569.023, 569.348, 569.671]
			v_pred_gurobi = [3.89683, 3.80328, 3.71432, 3.63315, 3.56147, 3.49982, 3.44789, 3.40483, 3.36947, 3.34047, 3.31653, 3.2964, 3.27898, 3.26335, 3.24878, 3.23473, 3.22085]
			#K_coeff = [0.106171, -179.742, 1.01431e5, -1.90795e7]
			K_coeff = [0.0,0.0,0.0,0.0]
			...
			for item in [df_opt_gurobi, df_pred_gurobi, ey_pred_gurobi, epsi_pred_gurobi, solv_time_lat_gurobi1, is_opt_lat_gurobi]
				print(item)
			end
			log_str_lat = @sprintf("Solve Status Lat. Gurobi: %s, SA: %.3f", is_opt_lat_gurobi, df_opt_gurobi)
			loginfo(log_str_lat)
