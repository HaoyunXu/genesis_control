...
module GPSKinMPCPathFollower
	...
	v_ref = ones(N+1)*1.0							    # target velocity for path follower
	x_ref   = 1.0*collect(0.0:dt:N*dt)	# target x(0.0 s), ..., x(N*dt s)
    ...
    function update_reference(x_ref::Array{Float64,1}, y_ref::Array{Float64,1}, psi_ref::Array{Float64,1}, v_ref::Array{Float64,1})
    	...
    	setvalue(v_target[i=1:(N+1)], v_ref)
    end
